...
public class TextFormatUtils {
    ...
    public static MutableComponent format(String key, Style style, Object... args) {
        ...
        while (matcher.find()) {
            ...
            if (matcher.start() > lastIndex) {
                result.append(Component.literal(raw.substring(lastIndex, matcher.start())).setStyle(style));
            }
            ...
            else result.append(Component.literal(matcher.group(0)).setStyle(style));

            lastIndex = matcher.end();
        }

        if (lastIndex < raw.length()) result.append(Component.literal(raw.substring(lastIndex)).setStyle(style));
        return result;
    }
}
